"""Lab browser aliases and display-chromium driver for urirdp-docker."""
...
def register(runtime: Runtime) -> None:
    aliases = [
        (
            ...
            "python://urirdpedge.lab_browser:open_page",
            "command",
            "browser.page.open",
            True,
        ),
        (
            ...
            "python://urirdpedge.lab_browser:get_dom",
            "query",
            "browser.page.dom",
            False,
        ),
        (
            ...
            "python://urirdpedge.lab_browser:screenshot",
            "command",
...
def _chromium_binary(profile: dict[str, Any]) -> str | None:
    for name in (profile.get("binary"), "chromium", "chromium-browser", "google-chrome"):
        if not name:
            continue
...
def _close_chromium(context: dict[str, Any]) -> None:
    ...
    subprocess.run(["pkill", "-f", "chromium"], env=env, capture_output=True, timeout=5, check=False)
    for wid in subprocess.run(
        ...
        wid = wid.strip()
        if wid:
            subprocess.run(["xdotool", "windowclose", wid], env=env, capture_output=True, timeout=3, check=False)
    time.sleep(0.8)
...
def open_page(payload: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
    ...
    args = [binary, "--no-sandbox", "--disable-dev-shm-usage", "--disable-gpu", f"--app={url}"]
